...
instance Applicative List where
  ...
  (<*>) ::
    ...
    -> List a
    -> List b
  (<*>) l1 l2 = flatten $ foldRight (\f acc -> (map f l2) :. acc) Nil l1
...
replicateA ::
  ...
  -> f a
  -> f (List a)
replicateA x = sequence . (replicate x)
...
filtering ::
  ...
  -> List a
  -> f (List a)
filtering p = foldRight (\x acc -> lift2 (\b -> ifThenElse b (x :.) (id)) (p x) acc) (pure Nil)
