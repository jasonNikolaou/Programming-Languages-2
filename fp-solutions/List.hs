...
-- implement map using composition (.)
...
filter ::
  ...
  -> List a
  -> List a
filter p l = foldRight (\x acc -> if p x then x :. acc else acc) Nil l
-- filter _ Nil = Nil
-- filter predicate (h :. t) = if (predicate h) then (h :. filter predicate t)
...
seqOptional l = let
                  helpFun Nil res = Full (reverse res)
                  helpFun (Empty :. _) _ = Empty
                  helpFun ((Full x) :. xs) res = helpFun xs (x :. res)
                in
                  helpFun l Nil
...
lengthGT4 ::
  ...
  -> Bool
lengthGT4 l = let
                helpFun Nil len = len > 4
                helpFun (_ :. xs) len =
                  if len > 4 then True
                  else helpFun xs (len+1)
              in
                helpFun l 0
-- lengthGT4 Nil = False
...
reverse ::
  List a
  -> List a
reverse l = foldLeft (flip (:.)) Nil l
-- reverse l = foldLeft (\acc x -> x :. acc) Nil l
